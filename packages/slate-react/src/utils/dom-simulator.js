...
function insertSimulatedNodeAfter(simulatedNode, added, after) {
  const index =
    after === null
      ? 0
      : simulatedNode.childNodes.findIndex(element => {
  ...
  if (mutation.target !== simulatedNode.realNode) {
    ...
    if (mutation.type === 'childList') {
      for (const added of mutation.addedNodes) {
        const simulatedAdded = findSimulatedNode(
          added,
          simulatedNode.childNodes
        )
      ...
      for (const removed of mutation.removedNodes) {
        const foundDetached = detached.findIndex(
          node => node.realNode === removed
        )
        const simulatedRemoved =
          foundDetached !== -1
            ? detached[foundDetached]
            : simulateDOMNode(removed)
        detached.splice(simulatedRemoved, 1)

        insertSimulatedNodeAfter(
          simulatedNode,
          simulatedRemoved,
