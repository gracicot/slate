...
function nodeAccountsForPath(node) {
  return (
    ...
    (node.dataset.slateObject === 'text' ||
      ...
      (node.nodeName !== '#text' && isRootNode(node.parentNode)))
  )
}
  ...
  if (node.childNodes.length > 0) {
    while (textNode === null && !end) {
      if (currentNode.dataset.slateObject === 'text') {
        ...
        if (
          (offset <= searchOffset &&
            lengthCurrentNode + offset > searchOffset) ||
          (!anchor && lengthCurrentNode + offset === searchOffset)
        ) {
...
function getRangeText(simulatedNode, diff) {
  if (nodeAccountsForPath(simulatedNode)) {
    ...
    const pointFocus =
      diff.start === diff.end
        ? pointAnchor
        : getPointByOffset(simulatedNode, diff.end, false)
    return {
      anchor: pointAnchor,
      focus: pointFocus,
    }
    ...
    const pointFocus =
      diff.start === diff.end
        ? pointAnchor
        : getPointForNode(simulatedNode, diff.end)
    return {
      anchor: pointAnchor,
      focus: pointFocus,
    }
...
function getRangeAddedNode(node, previousSibling, simulatedNodeSlateRoot) {
  ...
  if (!nodeAccountsForPath(currentNode)) {
    ...
    if (previousSibling !== null) {
      const previous = findSimulatedNodeRecursively(
        previousSibling,
        simulatedNodeSlateRoot
      )
...
export function processChildList(mutation, context) {
  ...
  if (
    mutation.target.dataset.slateEditor === 'true' ||
    mutation.target.dataset.slateObject === 'block'
  ) {
    if (
      mutation.addedNodes.length === 1 &&
      mutation.addedNodes[0].dataset &&
      mutation.addedNodes[0].dataset.slateObject === 'block'
    ) {
      const { range, index } = getRangeAddedNode(
        mutation.target,
  ...
  if (mutation.target.dataset.slateObject === 'block') {
    if (
      mutation.addedNodes.length === 1 &&
      mutation.addedNodes[0].dataset &&
      mutation.addedNodes[0].dataset.slateObject === 'text'
    ) {
      const { range, index } = getRangeAddedNode(
        mutation.target,
        mutation.previousSibling,
  ...
  if (mutation.target.dataset.slateObject === 'block') {
    if (
      ...
      mutation.removedNodes[0].dataset.slateObject === 'text'
    ) {
      if (
  ...
  if (
    ...
    if (
      mutation.removedNodes.length === 1 &&
      mutation.removedNodes[0].dataset &&
      mutation.removedNodes[0].dataset.slateObject === 'block'
    ) {
      const simulatedRemoved = findSimulatedNodeRecursively(
        mutation.removedNodes[0],
        simulatedNodeSlateRootBefore
...
function domPointToFurthestNode(domNode, domOffset, prefer) {
    ...
    while (domNode.childNodes.length !== 0) {
      if (prefer === 'right') {
        ...
        if (domOffset === 0) {
          ...
          domOffset = isEmptyNode(domNode)
            ? 0
            : domNode.childNodes.length || domNode.textContent.length
        } else {
  ...
  if (domNode.nodeName === 'BR') {
    const previousNode = domNode.previousSibling
...
function rangeNodeAndOffset(domRange) {
  ...
  if (
    nodeInEditor(domRange.startContainer) &&
    nodeInEditor(domRange.endContainer)
  ) {
    const [startNode, startOffset] = domPointToFurthestNode(
      domRange.startContainer,
      domRange.startOffset,
      'right'
    )
    const [endNode, endOffset] = domPointToFurthestNode(
...
export function getRangeFromDOMRange(staticRange) {
  ...
  if (nodeAndOffset !== null) {
    const anchor = getPointForNode(
      nodeAndOffset.anchor.node,
      nodeAndOffset.anchor.offset
    )
    const focus = getPointForNode(
      nodeAndOffset.focus.node,
      nodeAndOffset.focus.offset
    )
    return { anchor, focus }
  } else {
    return null
...
function findCompositionStart(compositionText, text, cursorInText) {
  ...
  if (compositionText.length === cursorInText)
    return cursorInText - compositionText.length
  ...
  const textAround = text.substring(
    regionBegin - beginOverflow,
    regionEnd - endOverflow
  )
...
export function findCompositionRange(event) {
  ...
  const compositionStart = findCompositionStart(
    event.data || '',
    range.focus.node.textContent,
    range.focus.offset
  )

  return {
    anchor: getPointForNode(range.anchor.node, compositionStart),
    focus: getPointForNode(
      range.focus.node,
      compositionStart + event.data.length
    ),
  }
}
