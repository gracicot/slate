...
function getDiffStart(prev, next, minEnd) {
  // const length = Math.min(prev.length, next.length)
  for (let i = 0; i < minEnd; i++) {
    if (prev.charAt(i) !== next.charAt(i)) return i
  }

  if (prev.length !== next.length) return minEnd;
  return null
}
...
function getDiffOffsets(prev, next) {
  ...
  const maxEnd = Math.min(prev.length, next.length)
  const end = getDiffEnd(prev, next, maxEnd)
  const minEnd = Math.min(prev.length - end, next.length - end)
  const start = getDiffStart(prev, next, minEnd)
