import { simulateDOMNodeBeforeMutations, getContentText, findSimulatedNodeRecursively, getNodeHtml } from '../dom-simulator'

function walkDOM(parent, iteration) {
	iteration(parent);

	if (parent.childNodes) {
		for (const node of parent.childNodes) {
			iteration(node);
			walkDOM(node, iteration);
		}
	}
}
...
export function indexedDOMSimulation(parent, nodes = [], index = new Map([])) {
	walkDOM(parent, node => {
		index.set(node.realNode, nodes.length);
		nodes.push(node.realNode);
	});
...
function recordMutationOperations(node, mutations, context) {
	...
	mutations.forEach((mutation, mutationIndex) => {
		...
		if (mutation.type === 'characterData') {
			...
			operations.push({type: 'setTextContent', data: {node: index.get(mutation.target), value: getContentText(simulated)}});
	...
	mutations.forEach((mutation, mutationIndex) => {
		...
		if (mutation.type === 'characterData') {
			serializedMutations.push({
				...
				oldValue: getContentText(stateBefore),
