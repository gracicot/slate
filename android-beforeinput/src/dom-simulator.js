...
function insertSimulatedNodeBefore(simulatedNode, added, before) {
	const index = before === null
		? simulatedNode.childNodes.length
		: simulatedNode.childNodes.findIndex(
			(element) => {
				return element.realNode === before;
			}
		);
	...
	if (mutation.target !== simulatedNode.realNode) {
		...
		if (mutation.type === 'childList') {
			for (const added of mutation.addedNodes) {
				...
				removeSimulatedNode(simulatedAdded);
			}
			...
			for (const removed of mutation.removedNodes) {
				...
				insertSimulatedNodeBefore(simulatedNode, simulatedRemoved, mutation.nextSibling);
			}
